...
function showSubWarningBanner(message) {
  ...
  banner.innerHTML = '⚠️ ' + message + ' <a href="mailto:hello@celerapps.com" style="color:#1a1a1a;text-decoration:underline;font-weight:800;margin-left:8px">Contact Support</a>' +
...
function showSubBlockedOverlay(message) {
  ...
  overlay.innerHTML = '<div style="text-align:center;max-width:400px;padding:40px">' +
    ...
    '<a href="mailto:hello@celerapps.com" style="display:inline-block;background:rgba(255,171,64,.2);color:#ffab40;border:1px solid rgba(255,171,64,.4);padding:12px 28px;border-radius:10px;font-weight:700;font-size:13px;text-decoration:none">📧 Contact CelerApps Support</a>' +
...
async function enterApp() {
  ...
  setTimeout(function () { if (typeof checkNewLeads === 'function') checkNewLeads(); }, 1500);
}
...
window.switchAdminTab = function (tab) {
  ['users', 'analytics', 'leaderboard', 'leads'].forEach(function (t) {
  ...
  if (ctx2) {
    var c2 = new Chart(ctx2, {
      ...
      data: {
        ...
        datasets: [{
          data: [totals.interested, totals.callback, totals.noanswer, totals.notinterested],
...
window.loadLeaderboard = async function (days) {
  ...
  try {
    ...
    var sorted = userIds.map(function (uid) {
      var u = byUser[uid];
      return {
        id: uid, name: nameMap[uid] || 'Rep', called: u.called, interested: u.interested,
...
// ════════════════════════════════════════════════════════
//  ADMIN — LEADS MANAGEMENT (Auto-Capture)
// ════════════════════════════════════════════════════════
window.loadAdminLeads = async function () {
  ...
  // Display webhook URL + secret
  if (currentTenant) {
    var urlEl = document.getElementById('webhookUrlDisplay');
  ...
  try {
    // Fetch leads with assigned rep names
    var query = _sb.from('leads')
      .select('*, user_profiles!leads_assigned_to_fkey(full_name, email)')
    ...
    res.data.forEach(function (lead, idx) {
      var tr = document.createElement('tr');
      ...
      tdPhone.style.fontSize = '11px';
      tdPhone.textContent = lead.phone || '—';
      tr.appendChild(tdPhone);

      // Source
      var tdSource = document.createElement('td');
      var sourceIcons = { facebook: '📘', website: '🌐', zapier: '⚡', indiamart: '🏪', api: '🔌', manual: '📝' };
      ...
      // Interest
      var tdInt = document.createElement('td');
      tdInt.textContent = lead.interest || '—';
      tdInt.style.fontSize = '11px';
      tdInt.style.color = 'var(--muted)';
      tdInt.style.maxWidth = '120px';
      tdInt.style.overflow = 'hidden';
      tdInt.style.textOverflow = 'ellipsis';
      tdInt.style.whiteSpace = 'nowrap';
      tr.appendChild(tdInt);
      ...
      var statusLabels = {
        ...
        'not_interested': '🔴 Not Interested', 'callback': '🔔 Callback', 'invalid': '⚠️ Invalid'
      };
      tdStatus.textContent = statusLabels[lead.status] || lead.status;
      tdStatus.style.fontSize = '11px';
      if (lead.status === 'interested') tdStatus.style.color = 'var(--green)';
      if (lead.status === 'not_interested') tdStatus.style.color = 'var(--danger)';
      tr.appendChild(tdStatus);

      // Assigned To
      var tdAssigned = document.createElement('td');
      var repProfile = lead.user_profiles;
      tdAssigned.textContent = repProfile ? (repProfile.full_name || repProfile.email || '—') : 'Unassigned';
      tdAssigned.style.fontSize = '11px';
      tr.appendChild(tdAssigned);

      // Received time
      var tdTime = document.createElement('td');
      tdTime.style.fontSize = '10px';
      tdTime.style.color = 'var(--muted)';
      if (lead.created_at) {
        var ago = timeAgo(new Date(lead.created_at));
        tdTime.textContent = ago;
        tdTime.title = new Date(lead.created_at).toLocaleString('en-IN');
    ...
    if (loadingEl) loadingEl.style.display = 'none';
    if (emptyEl) { emptyEl.style.display = 'block'; emptyEl.textContent = '❌ Error loading leads: ' + e.message; }
  }
};
...
window.copyWebhookUrl = function () {
  var el = document.getElementById('webhookUrlDisplay');
  if (el) { navigator.clipboard.writeText(el.value); showToast('📋 Webhook URL copied!'); }
};

// Webhook secret copy
window.copyWebhookSecret = function () {
  var el = document.getElementById('webhookSecretDisplay');
  if (el) { navigator.clipboard.writeText(el.value); showToast('📋 Webhook secret copied!'); }
};
...
window.showSnippetModal = async function () {
  var slug = currentTenant ? currentTenant.slug : 'YOUR_SLUG';
  var secret = currentTenant ? (currentTenant.webhook_secret || 'YOUR_SECRET') : 'YOUR_SECRET';
  var webhookUrl = SUPABASE_URL + '/functions/v1/webhook-leads?tenant=' + slug;

  var snippet = '<!-- DialKaro Lead Capture — paste before </body> -->\n' +
    '<script>\n' +
    '(function() {\n' +
    '  var WH = "' + webhookUrl + '";\n' +
    '  var SK = "' + secret + '";\n' +
    ...
    '      d.secret = SK;\n' +
    '      fetch(WH, { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify(d) })\n' +
    '        .then(function() { f.reset(); alert("Thank you! We will call you shortly."); })\n' +
    '        .catch(function() { alert("Something went wrong. Please try again."); });\n' +
    '    });\n' +
    '  });\n' +
    '})();\n' +
    '<\/script>\n\n' +
    '<!-- Add data-dialkaro to any form -->\n' +
    '<form data-dialkaro>\n' +
  ...
  await appAlert(snippet, '🌐');
};

// Zapier guide modal
window.showZapierGuide = async function () {
  var slug = currentTenant ? currentTenant.slug : 'YOUR_SLUG';
  var secret = currentTenant ? (currentTenant.webhook_secret || 'YOUR_SECRET') : 'YOUR_SECRET';
  var webhookUrl = SUPABASE_URL + '/functions/v1/webhook-leads?tenant=' + slug;

  var guide = '⚡ Zapier / Pabbly Setup Guide\n\n' +
    '1. Create a new Zap/Flow\n' +
    '2. Trigger: Choose your lead source (Facebook Lead Ads, Google Forms, TypeForm, etc.)\n' +
    '3. Action: "Webhooks by Zapier" → "POST"\n' +
    '4. URL: ' + webhookUrl + '\n' +
    '5. Payload Type: JSON\n' +
    '6. Data fields:\n' +
    '   • secret: ' + secret + '\n' +
    '   • name: {{lead name from step 1}}\n' +
    '   • phone: {{lead phone from step 1}}\n' +
    '   • email: {{lead email from step 1}}\n' +
    '   • interest: {{any custom field}}\n' +
    '   • source: zapier\n\n' +
    '7. Test & Turn On!\n\n' +
    'That\'s it! Leads will flow into DialKaro automatically.';

  await appAlert(guide, '⚡');
};
